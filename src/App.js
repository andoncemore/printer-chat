...
function App() {
  ...
  //     const blob = await getCanvasBlob(item.ref.current);
  //     formData.append("image", blob);
  //   }
  //   fetch('/print-elements',{
  //     method: 'POST',
  //     body: formData  
  //   })
  //   .then(res => res.json())
  ...
  const printReciept = async () => {
    ...
    let html_to_send = printList.map((item) => {
      if(item.type === 'image'){
  ...
  return (
    ...
    <div className="App">
      <div className="TitleBlock">
        <h1>printer chat</h1>
        <p>send images, text, and drawings to <mark>adit's reciept printer.</mark></p>
      </div>
      <div className="Options">
        <label className="filebtn">
          Upload Image
          <input type="file" accept="image/*" multiple onChange={(e) => handleFileUpload(e)} />
        </label>
        <button onClick={(elt) => addTextBlock()}>Aa</button>
        <button disabled={true}>Drawing</button>
        <button disabled={true}>Import</button>
        <button disabled={true} onClick={(elt) => handleArena()}>
      ...
      <div className="Contents" style={{fontSize:`${fontSize}px`}} ref={recieptEl}>
                      ...
                      if(elt.type === 'image'){
                        ...
                        return(
                          <TextEditor 
                            ...
                            dragHandleProps={provided.dragHandleProps}
                            content={elt.content}
                            updateContent={(value) => updateTextBlock(elt.id, value)}
                          />
                        )
                      }
                    }}
    ...
    {openModal !== "" &&
        <Modal title={openModal} closeModal={() => setOpenModal("")}>
          {openModal === "are.na import" &&
            <ArenaDialog closeModal={() => setOpenModal("")} addElements={addElements} />
          }
        </Modal>
      }
    </React.Fragment>
